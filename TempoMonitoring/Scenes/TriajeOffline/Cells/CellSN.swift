...
//  TempoMonitoring
//
//  Created by Jose Silva on 10/09/21.
//  Copyright © 2021 Sportafolio SAC. All rights reserved.
//

import UIKit
import SkyFloatingLabelTextField
class CellSN: UITableViewCell {
    ...
    func addItems(opciones: [DataPregunta], respuestas: [DataRepuesta?], cuadroTexto: String?) {
        ...
        do {
            ...
            for element in gRespuestas {
                ...
                btnOpc.setImages(empty: "radioEmpty", fill: "radioFill")
                btnOpc.titleLabel?.numberOfLines = 0
                btnOpc.titleLabel?.adjustsFontSizeToFitWidth = true
                ...
                let height = CalcHeightRow(width: mainView.layer.bounds.width - 125, text: element?.etiqueta ?? "")
                ...
                if element?.valor == gRespuestas.first??.valor {
                    ...
                    btnOpc.topAnchor.constraint(equalTo: lastView!.bottomAnchor, constant: 8).isActive = true
                }
                
                let alto = btnOpc.layer.bounds.height
            }
            if cuadroTexto != "" {
...
extension CellSN: CheckboxDelegate {
...
extension CellSN: UITextFieldDelegate {
